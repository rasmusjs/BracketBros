...
public class PostControllerTest
{
    ...
    private List<Post> GetMockPosts()
    {
        ...
        [
            new Post
            {
                ...
                Tags = mockTags, Comments = mockComments, TotalComments = 1
            },
            ...
            new Post
            {
                ...
                DateLastEdited = DateTime.Now, UserId = mockUser.Id, User = mockUser,
                CategoryId = 0, Category = null, TagsId = mockTagsId, Tags = mockTags, Comments = mockComments,
                TotalComments = 1
            },
            ...
            new Post
            {
                ...
                Tags = null, Comments = mockComments, TotalComments = 1
            }
        ];
    }
    ...
    private (ApplicationUser, ClaimsPrincipal) CreateMockUser()
    {
        ...
        var userClaims = new List<Claim>
        {
            ...
            new(ClaimTypes.Name, mockUser.UserName)
        };
    ...
    public async Task GetComments_ReturnCommentsOKTest()
    {
        ...
        _mockCommentRepository.Setup(repo => repo.GetAllCommentsByPostId(commentId))
            .ReturnsAsync(mockComments.Where(c => c.CommentId == commentId).ToList());
    ...
    public async Task GetComments_ReturnNotFoundTest()
    {
        ...
        _mockCommentRepository.Setup(repo => repo.GetAllCommentsByPostId(commentId))
            .ReturnsAsync(emptyComments.Where(c => c.CommentId == commentId).ToList());
    ...
    public async Task Create_HtmlSanitizerTest()
    {
        ...
        _mockPostRepository.Setup(repo => repo.Create(It.IsAny<Post>()))
            .Callback<Post>(p => Assert.DoesNotContain("<script>", p.Content)).ReturnsAsync(mockPost);
        _mockCategoryRepository.Setup(repo => repo.GetTById(It.IsAny<int>())).ReturnsAsync(mockCategory);
        _mockTags.Setup(repo => repo.GetTById(It.IsAny<int>())).ReturnsAsync(mockTag);
        _mockUserManager.Setup(repo => repo.FindByIdAsync(It.IsAny<string>())).ReturnsAsync(mockUser);
        // Mock successful user update
    ...
    public async Task Create_UserUpdateSucessTest()
    {
        ...
        _mockUserManager.Setup(m => m.FindByIdAsync(userId))
            .ReturnsAsync(mockUser); // Ensure FindByIdAsync returns the mock user
        _mockUserManager.Setup(m => m.UpdateAsync(It.IsAny<ApplicationUser>())).ReturnsAsync(IdentityResult.Success);
        ...
        _mockUserManager.Verify(m => m.UpdateAsync(It.Is<ApplicationUser>(u => u.Id == userId)),
            Times.Once); // Verify UpdateAsync was called
        // Verify the response
        var okResult = Assert.IsType<OkObjectResult>(result);
    ...
    public async Task Create_UserUpdateFailTest()
    {
        ...
        _mockUserManager.Setup(m => m.UpdateAsync(It.IsAny<ApplicationUser>()))
            .ReturnsAsync(IdentityResult.Failed()); // Fail user update
    ...
    public async Task Update_ReturnPostOkTest()
    {
        ...
        var capturedState = EntityState.Detached;
    ...
    public async Task UpdateComment_ReturnErrorUpdatingCommentTest()
    {
        ...
        // Mock repo
        _mockCommentRepository.Setup(repo => repo.GetTById(It.IsAny<int>())).ReturnsAsync(mockComment);
        _mockCommentRepository.Setup(repo => repo.Update(It.IsAny<Comment>()))
    ...
    public async Task DeleteComment_ReturnCommentNotFoundTest()
    {
        ...
        _mockCommentRepository.Setup(repo => repo.GetTById(It.IsAny<int>()))
            .ReturnsAsync((Comment)null!); // return null comment
