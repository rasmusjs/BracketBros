...
<template>
  <v-layout>
    <v-app-bar :height="display.mdAndUp.value ? '64' : '56'" elevation="3">
      <template v-if="display.smAndDown.value" v-slot:prepend>
        <v-app-bar-nav-icon
      ...
      <template v-if="display.mdAndUp.value">
        <v-divider class="h-25 my-auto mx-5" vertical></v-divider>
        ...
        <v-list nav>
          ...
          >
            <template v-slot:prepend>
              <v-icon :icon="page.icon" class="ml-1" size="small"></v-icon>
            </template>
